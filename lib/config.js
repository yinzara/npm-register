...
module.exports = {
  ...
  gcs: {
    bucket: env.NPM_REGISTER_GCS_BUCKET || env.GCS_BUCKET
  },
  npm: {
  ...
  auth: {
    ...
    key: env.NPM_REGISTER_CRYPTO_KEY,
    keyEncoding: env.NPM_REGISTER_CRYPTO_ENCODING || 'hex',
    algo: env.NPM_REGISTER_CRYPTO_ALGO || 'aes-128-cbc',
    ivLength: env.NPM_REGISTER_CRYPTO_IV_LENGTH || 16,
    oauth: {
      clientID: env.NPM_OAUTH_CLIENT_ID,
      clientSecret: env.NPM_OAUTH_CLIENT_SECRET,
      callbackURL: env.NPM_OAUTH_CALLBACK_URL || '/auth/google/callback'
    }
  },
...
let userType = ((env.NPM_REGISTER_USER && env.NPM_REGISTER_USER.toLowerCase()) || 'htpasswd')
let User = require('./user/' + userType)
